...
/**
 *  \file       inscriptions_contact.php
 *  \ingroup    college
 *  \brief      Tab for contacts linked to Inscriptions
 */
...
$hookmanager->initHooks(array('inscriptionscontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->college->inscriptions->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Inscriptions')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Inscriptions"), -1, $object->picto);
