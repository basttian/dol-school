...
/**
 *  \file       notes_contact.php
 *  \ingroup    college
 *  \brief      Tab for contacts linked to Notes
 */
...
$hookmanager->initHooks(array('notescontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->college->notes->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Notes')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Notes"), -1, $object->picto);
