...
/**
 *  \file       notes_document.php
 *  \ingroup    college
 *  \brief      Tab for documents linked to Notes
 */
...
$hookmanager->initHooks(array('notesdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->college->multidir_output[$object->entity ? $object->entity : $conf->entity]."/notes/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->college->notes->write; // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
$title = $langs->trans("Notes").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("Notes"), -1, $object->picto);
	...
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->college->notes->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;

	//$relativepathwithnofile='notes/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'notes/'.dol_sanitizeFileName($object->ref).'/';
