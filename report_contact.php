...
/**
 *  \file       report_contact.php
 *  \ingroup    college
 *  \brief      Tab for contacts linked to Report
 */
...
$hookmanager->initHooks(array('reportcontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->college->report->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Report')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Report"), -1, $object->picto);
