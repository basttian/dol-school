...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->college->multidir_output[$object->entity ? $object->entity : $conf->entity]."/report/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->rights->college->report->read;
	$permissiontoadd = $user->rights->college->report->write; // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
print dol_get_fiche_head($head, 'document', $langs->trans("Report"), -1, $object->picto);
...
$modulepart = 'college';
//$permissiontoadd = $user->rights->college->report->write;
$permissiontoadd = 1;
//$permtoedit = $user->rights->college->report->write;
$permtoedit = 1;
$param = '&id='.$object->id;

//$relativepathwithnofile='report/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'report/'.dol_sanitizeFileName($object->ref).'/';
