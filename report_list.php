...
/**
 *   	\file       report_list.php
 *		\ingroup    college
 *		\brief      List page for report
 */
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('college', 'report', 'write');
	$permissiontodelete = $user->hasRight('college', 'report', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Report|FR:Module_Report_FR|ES:Módulo_Report";
$help_url = '';
$title = $langs->trans("Reports");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/college/report_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/college/report_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
