...
/**
 *   	\file       students_card.php
 *		\ingroup    college
 *		\brief      Page to create/edit/view students
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'studentscard'; // To manage different context of search
...
$hookmanager->initHooks(array('studentscard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->college->students->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->rights->college->students->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->rights->college->students->write; // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->rights->college->students->write; // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->college->multidir_output[isset($object->entity) ? $object->entity : 1].'/students';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/college/students_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/college/students_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
  ...
  // Actions cancel, add, update, update_extras, confirm_validate, confirm_delete, confirm_deleteline, confirm_clone, confirm_close, confirm_setdraft, confirm_reopen
	...
	$triggersendname = 'COLLEGE_STUDENTS_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_STUDENTS_TO';
	$trackid = 'students'.$object->id;
...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Students")), '', 'object_'.$object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Students"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Students"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteStudents'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action != 'presend') {
      ...
      $langs->defaultlang,'',$object,0,'remove_file');
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/college/students_agenda.php', 1).'?id='.$object->id);
