...
/**
 *  \file       students_contact.php
 *  \ingroup    college
 *  \brief      Tab for contacts linked to Students
 */
...
$hookmanager->initHooks(array('studentscontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->college->students->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Students')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Students"), -1, $object->picto);
