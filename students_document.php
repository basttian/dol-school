...
/**
 *  \file       students_document.php
 *  \ingroup    college
 *  \brief      Tab for documents linked to Students
 */
...
$hookmanager->initHooks(array('studentsdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->college->multidir_output[$object->entity ? $object->entity : $conf->entity]."/students/".get_exdir(0, 0, 0, 1, $object);
}
...
$title = $langs->trans("Students").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("Students"), -1, $object->picto);
	...
	$relativepathwithnofile = 'students/'.dol_sanitizeFileName($object->ref).'/';
