...
/**
 *  \file       subject_contact.php
 *  \ingroup    college
 *  \brief      Tab for contacts linked to Subject
 */
...
$hookmanager->initHooks(array('subjectcontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->college->subject->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Subject')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Subject"), -1, $object->picto);
