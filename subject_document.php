...
$hookmanager->initHooks(array('subjectdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->college->multidir_output[$object->entity ? $object->entity : $conf->entity]."/subject/".get_exdir(0, 0, 0, 1, $object);
}
...
$title = $langs->trans("Subject").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("Subject"), -1, $object->picto);
	...
	$relativepathwithnofile = 'subject/'.dol_sanitizeFileName($object->ref).'/';
